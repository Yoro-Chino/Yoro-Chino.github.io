...
对于 AVG/Gal 的评分相对来说形成了一个比较成体系的体系，评价等级按照下表，大概就是这么个等级。（20/03/06 划分，20/03/18 修改）
...
注：因 9-nine 系列为剧情连贯性较强的分割放送，对每个分割放送单元给出一个作品分，并将每个放送单元作为一条线路计算整个系列的评分。因此上表中会将 9-nine 系列四作和系列整体分别列出，指向的实际上是同一个链接。ラブラブル系列同理。
