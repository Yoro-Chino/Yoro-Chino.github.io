...
对同一首歌曲重复导入新版本的 `.yzcpkg`（前提是它有一个**真实的、非默认的** `id`）：

- 新包会先解压到临时目录并完成基础校验，再替换旧目录；如果解压或替换失败，原有歌曲目录会保留。
- 替换成功后，旧谱面数据被完全覆盖（这是按 `id` 就地更新同一首歌的预期行为）。
- **游戏记录处理**：若某难度的音符数量未变，原最佳分数保留；若音符发生增减，该难度的分数清零，其余难度不受影响。
- **默认 `id` 例外**：如果包的 `id` 还是编辑器默认的 `chart`，导入时**绝不覆盖**已存在的 `chart` 歌曲，而是自动顺延为 `chart_2`、`chart_3`…（目录名与 `song.json` 的 `id` 一并改写）。因此不同协作者都忘记改 ID 时也不会互相覆盖。代价：用默认 `chart` 重复导入自己更新过的同一首歌，会变成一条新条目而非就地更新——请在编辑器里给歌曲填上 Title（ID 会自动派生成真实值）或手动设一个 `Song ID`。

## 包名与路径限制

- `song.json` 的 `id` 只能使用小写英文字母、数字和下划线（`[a-z0-9_]`）。
- `id` 不能与内置歌曲 ID 冲突。
- 默认 `id` `chart` 导入冲突时不会覆盖，而是顺延为 `chart_2`、`chart_3`…（见上节「默认 `id` 例外」）。
