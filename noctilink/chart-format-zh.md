...
**曲线段（可选）**：每个路径点还可以带 `curve_in` 字段描述该点的**入边段**形状——`"line"`（默认，省略不写）、`"bezier"` 或 `"arc"`。当 `curve_in` 为 `"bezier"` 或 `"arc"` 时，必须配套写 `cp_x` / `cp_y`（**参考点**，使用 chart 标准化坐标 0..1）。加载时无法识别的 `curve_in` 值一律按 `"line"` 处理（向前兼容——旧版本打开新曲线类型会退化成直线而非报错）。

- **`"bezier"`**：参考点是**曲线外**的二次 Bezier 控制点（曲线被它「拉拽」但不经过它）。公式：

  ```
  P(t) = (1-t)² · P_prev + 2t(1-t) · CP + t² · P_this   ,   t ∈ [0, 1]
  ```

- **`"arc"`**：参考点是曲线**上**的一点——线段是从上一个节点、**经过**参考点、到这个节点的圆弧（即 P_prev、CP、P_this 三点外接圆中包含 CP 的那段弧）。当三点共线时（参考点落在 P_prev→P_this 的直线上，外接圆半径无穷大），该段退化为直线。

首节点（index 0）没有入边段，其 `curve_in` 在加载时被强制为 `"line"`。游戏侧和编辑器侧对每种曲线采样一致——Bezier 用 `N_BEZIER_SAMPLES = 16`，arc 按**扫掠角自适应**（约每 3° 一段，宽弧也平滑、缓弧不浪费；共线 arc 退化为 2 点直线）。distance / centerline 投影 / 渲染都基于该采样的折线近似，保证预览与实机一致。拐角处用倒角圆弧平滑相接（极尖的拐角会跳过倒角，由曲线/线段本身定形）。

**中间节点的 `time` 字段**：仍然有意义——驱动轨迹上 guide light（指引亮点）沿轨迹的运动节奏。Guide light 在 `path[k].time` 时刻位于 path[k] 对应的渲染曲线位置，提供"现在应该划到哪里"的视觉参考。

**`on_time` 字段（可选，默认 `true`）**：每个路径点可显式写 `"on_time": false` 表示「该点不参与 autoplay / guide-light 的时间锚定」。当为 `false`，guide light 与编辑器预览体不会在该点的 `time` 锚定位置；而是把周围相邻的 on-time 节点对当作时间锚 A、B，按"经过时长比例 × A→B 的总弧长 = 目标弧长"匀速通过。典型用途：纯粹用来塑形的几何锚（Bezier 控制中段、急转弯等），其 `time` 是为画路径形状而填的而非节拍，标为非 on-time 可避免本体在那个时间点上视觉"卡顿"。首节点与末节点强制为 on-time（loader 自动校正）。判定窗口、得分、持有、末段判定、真节点 SFX 时机都*不受影响*——这个字段仅影响 autoplay 节奏。序列化器在值为 `true` 时省略字段，未启用该特性的旧谱字节级保留。
...
- **Hold / Ex-Hold 与 Slide / Ex-Slide（持续段）**：按独立**判定单元**计分——首节点 tap 一个单元；中段 `[起始 + 120ms, 结束]` 每 240ms 一段（段内出现过有效按压 → Pure+，否则 Break，slide 另需本体在轨迹上），各一个单元；slide 终点再加一个二值单元。谱面总物量 = 全部单元之和，长 hold/slide 像一串 drag 一样贡献多个判定。短于 300ms 的不产生中段。
