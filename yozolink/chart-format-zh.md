...
```json
{
  ...
  "path": [
    ...
    { "x": 0.5, "y": 0.3, "time": 4.5, "fake": true },
    { "x": 0.8, "y": 0.5, "time": 5.0 }
  ]
}
```

每个路径点可以带一个可选的 `fake` 布尔字段（默认 `false`）。`fake: true` 的路径点是"假节点 / 锚点"——只参与曲线塑形，不绘制轨迹圆点、经过时不发出音效、也不参与判定，适合在不强加点感的前提下让 slide 路径拐弯。首节点和末节点必须保持真节点：编辑器禁用对它们的切换，解析器会把误置的 `fake: true` 强制回 `false`。序列化器在 `false` 时省略该字段，所以旧谱面经过保存往返后字节不变。
