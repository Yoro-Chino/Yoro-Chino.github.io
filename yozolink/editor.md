...
- **N / D 分音符**：`D` 是「几分音符」的分母（每全音符的分割数，例如 4 = 四分、8 = 八分、16 = 十六分、12 = 三连音）；`N` 表示「该拍之后第几个 `1/D` 音」。例如 `1·2·1 / 12 分音符` = 第 1 小节第 2 拍之后的第 1 个十二分音（三连音）。以整数计数，三连音 / 非 4/4 / 非二分拍都能精确表示，不会出现除不尽的小数。
- **`D` 与 snap 解耦、按音符自动判定**：`D` **不**绑定全局 snap 颗粒度，而是用与「音符染色」相同的栅格判定算法按该音符实际所处的分度自动给出——落在三连音上就显示 `12`，不会被强行用 `16` 表示。`D` 也可手动改写，把同一时间换算到另一个分度上表示。
