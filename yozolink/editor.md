...
- **`D` 与 snap 解耦、按音符自动取最简**：`D` **不**绑定全局 snap 颗粒度，而是把该音符相对拍头的偏移化成**最简分数**自动给出——分母取标准音值集合 `{4, 6, 8, 12, 16, 24, 32, 48 …}` 中能整除该位置的**最小**那个（二分族最小 4、三连音族最小 6）：本可约到 1 分 / 2 分的位置归到 4 分，可约到 3 分的归到 6 分。所以拍正中显示 `0/4`，拍的 ⅔ 处显示 `1/6`（而不是没约分的 `2/12`）。`D` 也可手动改写，把同一时间换算到另一个分度上表示（此时不强制约分）。
