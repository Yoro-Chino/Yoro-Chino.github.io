...
- **播放头 scrub 快捷键**：`,` / `.`（及鼠标滚轮）按**当前 snap 刻度**前后移动播放头；`PgUp` / `PgDn` 按**小节**（全音符 / 4 拍，BPM-aware，不受 snap 设置影响）粗粒度前后跳，`PgUp` 往前、`PgDn` 往后。`←` / `→` 移动的是选中音符 / 节点的时间，不是播放头。
...
**限制放置范围**（设置弹窗「交互方式」下方，默认开启）：开启后，音符只能放置 / 拖动到内缩区域 `x ∈ [1/8, 7/8]`、`y ∈ [1/9, 8/9]`（即 16×9 栅格左右各留 2 格、上下各留 1 格），超出区域的位置会被**夹到边界内**。约束作用于音符头、slide 路径点与 hold 尾（Bézier 控制点不受限）。开启时 playfield 上会画出淡淡的允许区域边框。此外，开启时**叠 note 检测**会把任何落在区域外的音符（旧谱 / 导入谱里的边缘 note）标为**警告（⚠）**，只提示、不修改谱面。纯编辑器辅助，不改变谱面格式、不影响游玩；关闭后可自由放到边缘，警告也随之消失。
...
- **`D` 与 snap 解耦、按音符自动取最简**：`D` **不**绑定全局 snap 颗粒度，而是把该音符相对拍头的偏移化成**最简分数**自动给出——分母取能整除该位置的**最小整数**（最高到 192 分），再按下限规整：本可约到 1 分 / 2 分的位置归到 4 分，可约到 3 分的归到 6 分；**4 分及以上原样显示**（含五连音 `5`、七连音 `7` 等奇数连音）。所以拍正中显示 `0/4`，拍的 ⅔ 处显示 `1/6`（而不是没约分的 `2/12`），五分线上的音符显示 `1/5`。`D` 也可手动改写，把同一时间换算到另一个分度上表示（此时不强制约分）。
...
编辑器持续扫描当前难度内有重叠风险的 note 对，把计数显示在顶部工具栏「垂直镜像」右侧的徽章按钮上（也通过 `编辑 → 检测叠 note...` 入口）。每次新增、删除、移动 note 后自动重算。
...
点击工具栏徽章或菜单项弹出列表对话框，按 error→warning、再按时间排序。点击列表中的一行：playhead 跳到 A 的时间，A 被单选（B 在同一时间段内显示在 playfield 中，便于人工判断）。
