...
- **Slide 假节点（锚点）**：Inspector 每一行路径点都有一个「锚」复选框。勾选后该路径点变成假节点——只塑形曲线，不绘制轨迹圆点、不发出经过音、不参与判定。轨迹会在假节点拐角处自动圆滑过渡。**首节点与末节点必须保持真节点**，对应复选框被禁用。选中后 Slide 时假节点的选择标记会带一个内部「×」，与真节点区分。
